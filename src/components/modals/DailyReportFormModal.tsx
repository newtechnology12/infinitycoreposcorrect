...
export function DailyReportFormModal({
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const userRes = await pocketbase.collection("users").getOne(user.id);

    if (!userRes?.department) return toast.error("User department not found");

    const q = !record
      ? pocketbase.collection("daily_reports").create({
          ...data,
          employee: userRes?.id,
          department: userRes?.department,
        })
      : pocketbase.collection("daily_reports").update(record.id, data);

    return q
      .then(() => {
        onComplete();
        toast.error(
          q ? "Report updated succesfully" : "Report created succesfully"
        );
        form.reset();
      })
      .catch((e) => {
        ...
        <DialogHeader>
          <DialogTitle>
            <span className="text-base px-2 font-semibold py-2">
              {record ? "Update" : "Create a new"} Daily report.
            </span>
          </DialogTitle>
          <DialogDescription>
            <span
