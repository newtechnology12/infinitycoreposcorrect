...
export function RawItemsModal({ open, setOpen, onSelect, stock }: any) {
...
function StockItems({ onSelect, selected, setselected, stock }: any) {
  ...
  const stockItemsQuery = useQuery({
    queryKey: ["raw_items", { search: debouncedSearch, stock }],
    keepPreviousData: true,
    queryFn: async () => {
      const searchQ = `name~"${debouncedSearch}" || menu.name~"${debouncedSearch}"`;
